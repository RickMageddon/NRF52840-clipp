 ...
 * Features: Deep sleep power management, boot screen, BLE connection status
 */
...
void showShuttingDown() {
  ...
  // Center "Shutting Down" on display
...
void enterDeepSleep() {
  ...
  // nRF52840 can wake from System OFF using GPIO SENSE
  ...
  // Enter System OFF (lowest power mode, ~0.4µA)
  // Device will reset when button is pressed
...
void buttonTask(void *pvParameters) {
  ...
  // Long press detection for power management
  unsigned long buttonPressStartTime = 0;
  bool buttonHeld = false;
  bool longPressTriggered = false;
  
  // Track previous connection state for display updates
  ...
  while (1) {
    ...
    if (deviceState == STATE_SHUTDOWN_PROMPT) {
      // In shutdown prompt - waiting for user response
      ...
      if (currentButtonState == HIGH && lastButtonState == LOW) {
        if (buttonHeld && !longPressTriggered) {
          // Short press (single click) = Yes, shutdown
      ...
      // Check for long press (No, cancel shutdown)
    ...
    // Detect button press start (falling edge)
    if (currentButtonState == LOW && lastButtonState == HIGH) {
      buttonPressStartTime = millis();
      buttonHeld = true;
      longPressTriggered = false;
    ...
    // Check for long press (5 seconds) to trigger shutdown prompt
    if (buttonHeld && !longPressTriggered && (millis() - buttonPressStartTime) >= LONG_PRESS_TIME) {
      longPressTriggered = true;
    ...
    if (currentButtonState == HIGH && lastButtonState == LOW) {
      ...
      // Only process normal clicks if not a long press
      if (!longPressTriggered && deviceState == STATE_CONNECTED) {
    ...
    // ============== SINGLE-CLICK PROCESSING (after wait time) ==============
...
void setup() {
  ...
  Serial.begin(115200);
  // Don't wait for Serial - device should work without USB connected
  delay(100);
  ...
  showBootScreen();
  delay(2000);  // Show CLIPP for 2 seconds
  ...
  Serial.println("BLE advertising as 'CLIPP'...");
  ...
  xTaskCreate(
    ...
    "Button",          // Task name
    512,               // Stack size (increased for state machine)
    NULL,              // Parameters
    1,                 // Priority (low)
